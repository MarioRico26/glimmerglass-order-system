...
export default function AdminOrdersPage() {
    ...
    // Query state (sin estado duplicado: fuente de verdad = URL)
    const q            = sp.get('q') || ''
    ...
    // Agrupado (en cliente; si prefieres server, se puede mover)
    const [groupBy, setGroupBy] = useState<'DEALER' | 'FACTORY'>('DEALER')
    const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({})

    useEffect(() => {
        // abre por defecto todos los grupos
    ...
    // Paginación UI
    const total = Number(sp.get('total') || 0) // opcional si quisieras reflejarlo en URL
    // mejor: usa el count que devolvió la API vía cabecera… o añade estado local.
    // Para mantenerlo simple, pedimos otra llamada: ya viene en el payload.
    // Ajuste: guardamos total en un ref local de la última respuesta:
    const [totalCount, setTotalCount] = useState(0)
    useEffect(() => {
        // obtenemos total de la misma llamada load(); aquí sólo lo extraemos si vino en cache
        // por simplicidad, hacemos una consulta liviana:
        ;(async () => {
            const params = new URLSearchParams()
            if (q) params.set('q', q)
            if (statusFilter !== 'ALL') params.set('status', statusFilter)
                                            ...
                                            <table className="min-w-full text-sm">
                                                ...
                                                {list.map(order => (
                                                    <tr key={order.id} className="border-t">
                                                        <td className="py-2 px-3">{order.poolModel?.name || '-'}</td>
                                                        ...
                                                        <td className="py-2 px-3">
                                                            <Link href={`/admin/orders/${order.id}/history`}
                                                                ...
                                                                <PackageSearch size={16} /> View History
                                                            </Link>
                                                        </td>
                                                    </tr>
                                                ))}
